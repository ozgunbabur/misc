...
public class Grader
{
	...
	{
		String dir = "/Users/ozgun/Documents/Teaching/CS220/grades-fall-2020/";
		Map<String, Double> grades = getOverallGrades(dir + "CS_220_Fall_2020_grades.csv", dir + "late-list.txt", dir + "UMassBostonCS220BaburFall2020_assignment_report_2020-12-22_2157.csv");

		grades.keySet().stream().sorted(Comparator.comparing(o -> o.substring(o.lastIndexOf(" ") + 1))).forEach(name -> System.out.println(getLetterGrade(grades.get(name)) + "\t" + grades.get(name) + "\t" + name));
	}
	...
	private static Map<String, Double> getOverallGrades(String gradescopeFile, String latesFile, String zyBookFile) throws IOException
	{
		...
		weightMap.put("Homework #1", 0.04);
		weightMap.put("Homework #2", 0.04);
		weightMap.put("Midterm", 0.25);
		weightMap.put("Homework #3", 0.04);
		weightMap.put("Homework #4", 0.04);
		weightMap.put("Homework #5", 0.04);
		weightMap.put("Extra credit feedback", 1D);
		weightMap.put("Extra Credit Assignment", 0.04);
		weightMap.put("Final Exam", 0.4);

		Map<String, Double> curveMap = new HashMap<>();
		curveMap.put("Midterm", 15D);
		...
		Files.lines(Paths.get(gradescopeFile)).skip(1).map(l -> l.split(",")).forEach(t ->
		{
			...
			double grade = 0;
			grade += zyBookGrades.getOrDefault(name, 0D) * 0.15;

			for (String ass : weightMap.keySet())
			{
				int index = ArrayUtil.indexOf(header, ass);
				double x = t[index].isEmpty() ? 0 : Double.parseDouble(t[index]);
				if (x > 100) x = 100;
